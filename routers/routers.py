...
@router.get("/fetch_insights_adoreaboo")
def fetch_insights_adoreaboo(db: Session = Depends(get_db)):
    """
    ...
    """
    try:
        global ADOREABOO_ACCESS_TOKEN

        # Refresh the short-lived token
        if is_access_token_expired(ADOREABOO_ACCESS_TOKEN):
            try:
                refreshed_token = refresh_access_token(APP_ID, APP_SECRET, LONG_LIVED_TOKEN)
                set_key('.env', 'ADOREABOO_ACCESS_TOKEN', refreshed_token)
                load_dotenv()
                ADOREABOO_ACCESS_TOKEN = os.getenv("ADOREABOO_ACCESS_TOKEN")
                ...
                try:
                    ...
                    load_dotenv()
                    new_adoreaboo_access_token = refresh_access_token(APP_ID, APP_SECRET, new_long_lived_token)
                    set_key('.env', 'ADOREABOO_ACCESS_TOKEN', new_adoreaboo_access_token)
                    load_dotenv()
                    ADOREABOO_ACCESS_TOKEN = os.getenv("ADOREABOO_ACCESS_TOKEN")
        ...
        account_url = f"{BASE_URL}{ADOREABOO_INSTAGRAM_ACCOUNT_ID}?fields=id,username,followers_count&access_token={ADOREABOO_ACCESS_TOKEN}"
        ...
        # Fetch insights
        insights_url = f"{BASE_URL}{ADOREABOO_INSTAGRAM_ACCOUNT_ID}/insights?metric=reach,accounts_engaged,website_clicks&period=day&metric_type=total_value&access_token={ADOREABOO_ACCESS_TOKEN}"
    ...
    try:
        ...
        if is_access_token_expired(ADOREABOO_ACCESS_TOKEN):
            try:
                ...
                ADOREABOO_ACCESS_TOKEN = os.getenv("ADOREABOO_ACCESS_TOKEN")  # Get updated token
            except Exception as e:
                try:
                    new_long_lived_token = generate_new_long_lived_token()
                    set_key('.env', 'LONG_LIVED_TOKEN', new_long_lived_token)
                    load_dotenv()  # Reload the updated .env file
                    ADOREABOO_ACCESS_TOKEN = refresh_access_token(APP_ID, APP_SECRET, new_long_lived_token)
                    set_key('.env', 'ADOREABOO_ACCESS_TOKEN', ADOREABOO_ACCESS_TOKEN)
                    load_dotenv()  # Reload the updated .env file
                except Exception as gen_error:
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        # Define the API URLs
        insights_url = f"{BASE_URL}{ADOREABOO_INSTAGRAM_ACCOUNT_ID}/insights"
        params = {
            "metric": "engaged_audience_demographics",
            "period": "lifetime",
            "timeframe": "this_week",
            "metric_type": "total_value",
            "access_token": ADOREABOO_ACCESS_TOKEN,
        }
...
async def fetch_all_posts(db: Session = Depends(get_db)):
    try:
        ...
        if is_access_token_expired(ADOREABOO_ACCESS_TOKEN):
            try:
                refreshed_token = refresh_access_token(APP_ID, APP_SECRET, LONG_LIVED_TOKEN)
                os.environ["ADOREABOO_ACCESS_TOKEN"] = refreshed_token
                ADOREABOO_ACCESS_TOKEN = refreshed_token
            except Exception:
                new_long_lived_token = generate_new_long_lived_token()
                os.environ["LONG_LIVED_TOKEN"] = new_long_lived_token
                ADOREABOO_ACCESS_TOKEN = refresh_access_token(APP_ID, APP_SECRET, new_long_lived_token)
                os.environ["ADOREABOO_ACCESS_TOKEN"] = ADOREABOO_ACCESS_TOKEN

        # Fetch all posts
        all_posts = []
        posts_url = f"{BASE_URL}{ADOREABOO_INSTAGRAM_ACCOUNT_ID}/media"
        params = {
        ...
        metrics = await process_posts_async(all_posts, ADOREABOO_ACCESS_TOKEN)
